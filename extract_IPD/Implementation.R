TSD <- read.csv("D:/Dropbox/ExtractIPD/process/F_OS_CPS_1_pembrolizumab.csv")
digizeit <- DIGI.CLEANUP(TSD)

#DF <- read.csv("D:/Dropbox/ExtractIPD/process/AR.csv")
#AR <-filter(DF, Slide == 'A', Arm == 'pembrolizumab')
AR <- read.csv("D:/Dropbox/ExtractIPD/process/AR.csv")
...
write.csv(IPD, paste(paste('D:/Dropbox/ExtractIPD/process', nm, sep='/'), 'csv', sep='.'), row.names = FALSE)
